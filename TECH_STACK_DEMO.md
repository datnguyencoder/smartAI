...
## 9️⃣ Uptime Kuma (Availability Monitoring)

### Tech Details
- **Purpose:** Endpoint uptime monitoring & alerting
- **Check Interval:** 60 seconds
- **Database:** SQLite (embedded)
- **Notifications:** Telegram, Email, Webhook
- **Monitored Services:** 6 total
...
# 2. Dashboard view
# Should see 6 monitors:
# 1. Backend API — http://backend:8080/actuator/health (UP)
# 2. AI Service — http://ai-service:8000/ai/health (UP)
# 3. PostgreSQL — backend:5432 (UP)
# 4. Redis — backend:6379 (UP)
# 5. Frontend — https://smart-ai-five.vercel.app (UP)
# 6. Grafana — http://backend:3000/api/health (UP)

# 3. Click individual monitor for details
# - Response time: 45ms average
# - Uptime: 99.99%
# - Status: UP (green)

# 4. Status page
# Public status dashboard (optional)

# 5. Settings → Notifications
# Check Telegram bot configured

# 6. Query database
docker exec smartmart_uptime_kuma sqlite3 /app/data/kuma.db \
  "SELECT id, name, type, url, status FROM monitor LIMIT 10;"

# Expected output:
#  id |      name      | type    |                 url                 | status
# ────┼────────────────┼─────────┼─────────────────────────────────────┼────────
#   1 | Backend API    | keyword | http://backend:8080/actuator/health |   1
#   2 | AI Service     | keyword | http://ai-service:8000/ai/health    |   1
#   3 | PostgreSQL     | port    | postgres:5432                       |   1
#   4 | Redis          | port    | redis:6379                          |   1
#   5 | Frontend       | http    | https://smart-ai-five.vercel.app    |   1
#   6 | Grafana        | http    | http://backend:3000/api/health      |   1
```
...
- [ ] Uptime Kuma showing all monitors UP
