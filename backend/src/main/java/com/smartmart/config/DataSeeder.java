        ...
        private void seedUsers() {
                userRepository.saveAll(List.of(
                                User.builder().username("admin").password(passwordEncoder.encode("admin123"))
                                                .email("admin@smartmart.com").fullName("Quản trị viên")
                                                .role(Role.ROLE_ADMIN).status(UserStatus.ACTIVE).build(),
                                User.builder().username("staff").password(passwordEncoder.encode("staff123"))
                                                .email("staff@smartmart.com").fullName("Thu ngân").role(Role.ROLE_STAFF)
                                                .status(UserStatus.ACTIVE).build(),
                                User.builder().username("warehouse").password(passwordEncoder.encode("warehouse123"))
                                                .email("warehouse@smartmart.com").fullName("Nhân viên kho")
                                                .role(Role.ROLE_WAREHOUSE).status(UserStatus.ACTIVE).build()));
        }

        private void seedMasterAndInventory() {
                Uom cai = uomRepository.save(
                                Uom.builder().uomName("Cái").conversionRatio(BigDecimal.ONE).baseUnit(true).build());
                Uom thung = uomRepository.save(Uom.builder().uomName("Thùng").conversionRatio(new BigDecimal("24"))
                ...
                Category doUong = categoryRepository.save(Category.builder()
                                ...
                                .build());
                Supplier vinamilk = supplierRepository
                                .save(Supplier.builder().supplierName("Vinamilk").phone("1900").active(true).build());
                Location khoBan = locationRepository.save(
