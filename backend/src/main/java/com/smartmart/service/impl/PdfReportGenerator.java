...
public class PdfReportGenerator {
    ...
    public byte[] generateSalesReport(List<SalesReportResponse> data, LocalDate from, LocalDate to) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            Paragraph period = new Paragraph(String.format("Kỳ báo cáo: %s đến %s", from, to), getVietnameseFont(12, Font.NORMAL));
    ...
    private void generateCoverPage(Document document, LocalDate from, LocalDate to) throws DocumentException {
        ...
        Paragraph generatedTime = new Paragraph("Thời gian xuất: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm dd/MM/yyyy")), metaFont);
        generatedTime.setAlignment(Element.ALIGN_CENTER);
        document.add(generatedTime);
    }

    private void generateExecutiveDashboard(Document document, List<SalesReportResponse> sales, List<PurchaseReportResponse> purchases, List<InventoryReportResponse> inventory) throws DocumentException {
        ...
        // Calculate KPIs
        double totalRevenue = sales.stream().mapToDouble(r -> r.getTotalRevenue() != null ? r.getTotalRevenue().doubleValue() : 0).sum();
        double totalProfit = sales.stream().mapToDouble(r -> r.getGrossProfit() != null ? r.getGrossProfit().doubleValue() : 0).sum();
        double totalSpend = purchases.stream().mapToDouble(r -> r.getTotalAmount() != null ? r.getTotalAmount().doubleValue() : 0).sum();
        double totalStockValue = inventory.stream().mapToDouble(r -> r.getCurrentStock() != null ? r.getCurrentStock().doubleValue() * 1000 : 0).sum(); // Simulated value
        ...
        addKpiCell(kpiTable, "TỔNG DOANH THU", String.format("%,.0f VNĐ", totalRevenue), "▲ +12.4%", new Color(230, 245, 230), kpiLabelFont, kpiValueFont);
        addKpiCell(kpiTable, "LỢI NHUẬN GỘP", String.format("%,.0f VNĐ", totalProfit), "▲ +8.2%", new Color(230, 245, 230), kpiLabelFont, kpiValueFont);
        addKpiCell(kpiTable, "TỔNG CHI NHẬP HÀNG", String.format("%,.0f VNĐ", totalSpend), "▼ -3.1%", new Color(245, 230, 230), kpiLabelFont, kpiValueFont);
        addKpiCell(kpiTable, "GIÁ TRỊ TỒN KHO", String.format("%,.0f VNĐ", totalStockValue), "▲ +5.0%", new Color(255, 245, 230), kpiLabelFont, kpiValueFont);
        ...
        if (totalRevenue > totalSpend) {
            insights.add("Dòng tiền dương: Doanh thu vượt quá chi phí nhập hàng, cho thấy hiệu quả chuyển đổi vốn tốt.");
        } else {
            insights.add("CẢNH BÁO: Chi phí nhập hàng đang cao hơn doanh thu trong kỳ, cần kiểm soát lượng tồn kho.");
        }
        
        long nearExpiryCount = inventory.stream().filter(r -> r.getDaysUntilExpiry() != null && r.getDaysUntilExpiry() <= 30).count();
        if (nearExpiryCount > 0) {
            insights.add(String.format("Tồn kho rủi ro: Có %d mặt hàng sẽ hết hạn trong vòng 30 ngày tới. Yêu cầu xả hàng ngay lập tức.", nearExpiryCount));
        }
    ...
    private void addKpiCell(PdfPTable table, String label, String value, String delta, Color bgColor, Font labelFont, Font valueFont) {
        ...
        if (delta.contains("▼")) deltaFont.setColor(Color.RED);
        else deltaFont.setColor(new Color(0, 150, 0));
    ...
    private void generateSalesSection(Document document, List<SalesReportResponse> sales) throws DocumentException, IOException {
        ...
        for (SalesReportResponse s : sales) {
            trendData.addValue(s.getTotalRevenue() != null ? s.getTotalRevenue().doubleValue() : 0, "Doanh Thu", s.getPeriod());
        }
        ...
        for (SalesReportResponse row : sales) {
            ...
            table.addCell(new Phrase(row.getTotalRevenue() != null ? String.format("%,.0f", row.getTotalRevenue()) : "0", normalFont));
            table.addCell(new Phrase(row.getGrossProfit() != null ? String.format("%,.0f", row.getGrossProfit()) : "0", normalFont));
            table.addCell(new Phrase(String.format("%,d", row.getTotalItemsSold()), normalFont));
        }
        document.add(table);
    }

    private void generatePurchaseSection(Document document, List<PurchaseReportResponse> purchases) throws DocumentException, IOException {
        Font headerFont = getVietnameseFont(16, Font.BOLD);
        Font normalFont = getVietnameseFont(11, Font.NORMAL);
        ...
        List<PurchaseReportResponse> sorted = purchases.stream()
                .sorted(Comparator.comparing(PurchaseReportResponse::getTotalAmount, Comparator.nullsLast(Comparator.reverseOrder())))
                .collect(Collectors.toList());
        
        double otherAmount = 0;
        for (int i = 0; i < sorted.size(); i++) {
            double amt = sorted.get(i).getTotalAmount() != null ? sorted.get(i).getTotalAmount().doubleValue() : 0;
            if (i < 5) pieData.setValue(sorted.get(i).getSupplierName(), amt);
            else otherAmount += amt;
        }
        if (otherAmount > 0) pieData.setValue("Khác", otherAmount);
        ...
        for (PurchaseReportResponse row : purchases) {
            ...
            table.addCell(new Phrase(String.format("%,d", row.getTotalOrders()), normalFont));
            table.addCell(new Phrase(row.getTotalAmount() != null ? String.format("%,.0f", row.getTotalAmount()) : "0", normalFont));
            table.addCell(new Phrase(row.getTotalQuantity() != null ? String.format("%,.0f", row.getTotalQuantity()) : "0", normalFont));
        }
        document.add(table);
    }

    private void generateInventorySection(Document document, List<InventoryReportResponse> inventory) throws DocumentException, IOException {
        Font headerFont = getVietnameseFont(16, Font.BOLD);
        Font normalFont = getVietnameseFont(11, Font.NORMAL);
        ...
        long safe = inventory.stream().filter(r -> r.getCurrentStock() != null && r.getCurrentStock().doubleValue() > 10).count();
        long warning = inventory.stream().filter(r -> r.getCurrentStock() != null && r.getCurrentStock().doubleValue() > 0 && r.getCurrentStock().doubleValue() <= 10).count();
        long oos = inventory.stream().filter(r -> r.getCurrentStock() == null || r.getCurrentStock().doubleValue() == 0).count();
        ...
        for (InventoryReportResponse row : inventory) {
            if (row.getDaysUntilExpiry() != null && row.getDaysUntilExpiry() <= 30) {
                ...
                table.addCell(new Phrase(row.getCurrentStock() != null ? String.format("%,.0f", row.getCurrentStock()) : "0", normalFont));
                
                PdfPCell expiryCell = new PdfPCell(new Phrase(row.getDaysUntilExpiry() + " ngày", normalFont));
                if (row.getDaysUntilExpiry() <= 7) expiryCell.setBackgroundColor(new Color(255, 204, 204)); // Critical
    ...
    private void generateDataDictionaryAndSignatures(Document document) throws DocumentException {
        ...
        String[][] terms = {
            ...
            {"Thất thoát (Shrinkage)", "Sự khác biệt giữa tồn kho trên hệ thống và tồn kho thực tế, thường do mất cắp, hư hỏng, hoặc lỗi kiểm kê."},
            {"Tỷ lệ vòng quay (Turnover Rate)", "Số lần bán và thay thế hàng tồn kho trong một khoảng thời gian nhất định (Tổng bán / Tồn hiện tại)."}
        };
    ...
    class ReportHeaderFooterEvent extends PdfPageEventHelper {
        ...
        public void onEndPage(PdfWriter writer, Document document) {
            ...
            String text = footerText + " | Trang " + writer.getPageNumber() + " | " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm"));
