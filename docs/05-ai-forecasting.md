...
1.  **Gọi API:** Spring Boot Backend gọi API `POST /ai/forecast/all` sang FastAPI, gửi toàn bộ SKU `ACTIVE`. SKU chưa có giao dịch trong 90 ngày vẫn được gửi với `recent_sales = []` để AI dùng moving average/category baseline thay vì bị bỏ khỏi forecast.
...
4.  **Fallback khi AI offline:**
    *   Nếu FastAPI timeout hoặc không kết nối được, Backend không làm lỗi luồng vận hành.
    *   Hệ thống tính lại `reorder_recommendations` bằng lượng bán trung bình 30 ngày, đánh dấu `source = FALLBACK`.
