...
| :--- | :--- | :--- | :--- | :--- |
| **TC-RBAC-01** | `GET /api/v1/users` | `STAFF` | None (Gửi kèm Token JWT của STAFF) | HTTP **403 Forbidden**.<br>Không trả về danh sách người dùng. |
| **TC-RBAC-02** | `GET /api/v1/users` | `ADMIN` | None (Gửi kèm Token JWT của ADMIN) | HTTP **200 OK**.<br>Trả về danh sách tài khoản nhân viên. |
| **TC-RBAC-03** | `POST /api/v1/categories` | `STAFF` | `{ "name": "Đồ gia dụng" }` | HTTP **403 Forbidden**.<br>Không tạo được danh mục mới. |
| **TC-RBAC-04** | `POST /api/v1/categories` | `WAREHOUSE` | `{ "name": "Đồ gia dụng" }` | HTTP **201 Created**.<br>Tạo danh mục thành công. |
| **TC-RBAC-05** | `POST /api/v1/orders`| `WAREHOUSE` | `{ "items": [{"itemId":1, "quantity":1}] }`| HTTP **403 Forbidden**.<br>Nhân viên kho không được phép bán hàng. |
| **TC-RBAC-06** | `POST /api/v1/forecast/train`| `MANAGER` | None | HTTP **202 Accepted**.<br>Kích hoạt huấn luyện AI async thành công và trả `jobId`. |
...
| Mã Test Case | Tên Kịch bản kiểm thử | Mô tả các bước thực hiện | Kết quả mong đợi (Expected Output) |
| :--- | :--- | :--- | :--- |
| **TC-PUR-01**| **Tạo phiếu nhập thành công (chưa cộng tồn)** | 1. Chọn sản phẩm ID 8 (Mì ăn liền Hảo Hảo - `has_expiry = false`).<br>2. Tạo phiếu nhập 500 gói mì, không điền `expiryDate`. | • HTTP **201 Created**.<br>• Phiếu ở trạng thái `PENDING`.<br>• Tồn kho thực tế chưa thay đổi.<br>• Chưa có ledger cộng tồn. |
| **TC-PUR-01B**| **Nhận hàng phiếu nhập** | 1. Lấy phiếu `PENDING` vừa tạo.<br>2. Gọi `POST /api/v1/purchase-orders/{id}/receive`. | • HTTP **200 OK**.<br>• Phiếu chuyển `COMPLETED`.<br>• Tồn kho tăng theo số lượng quy đổi base UOM.<br>• Có `inventory_logs` action `PURCHASE_RECEIVE`. |
...
1.  **Bước 1 (Trạng thái bình thường):**
    *   Gọi API `GET /api/v1/forecast/recommendations`.
...
3.  **Bước 3 (Thử nghiệm Fallback):**
    *   Gọi `POST /api/v1/forecast/run`, sau đó gọi lại API `GET /api/v1/forecast/recommendations` trên React Frontend.
    *   *Kết quả mong đợi:*
