...
export function createScrapOrder(payload: {
  ...
  return apiRequest<ScrapOrderDto>('/api/v1/scrap-orders', {
    method: 'POST',
    body: JSON.stringify(payload),
  });
}
...
export function approveScrapOrder(id: number) {
  return apiRequest<ScrapOrderDto>(`/api/v1/scrap-orders/${id}/approve`, { method: 'POST' });
}

export function cancelScrapOrder(id: number, reason: string) {
  const params = new URLSearchParams({ reason });
  return apiRequest<ScrapOrderDto>(`/api/v1/scrap-orders/${id}/cancel?${params}`, { method: 'POST' });
}
